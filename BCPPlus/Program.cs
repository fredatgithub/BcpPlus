...
namespace BCPPlus
{
  class Program
  {
    ...
    private static void Usage()
    {
      ...
      display($"BcpPlus is a console application written by Freddy Juhel.");
      display($"BcpPlus.exe is in version {GetAssemblyVersion()}");
      display("BcpPlus needs Microsoft .NET framework 4.0 to run, if you don't have it, download it from microsoft.com.");
      display($"Copyrighted (c) 2017 by Freddy juhel, all rights reserved.");
      display(string.Empty);
      ...
      display("Examples:");
      display(string.Empty);
      display("BcpPlus /filename:MyCSVFile.txt /separator:, /hasheader:true /hasfooter:true /deleteheader:true /deletefooter:true /deletefirstcolumn:true /log:true");
      display(string.Empty);
      display("BcpPlus /help (this help)");
      display("BcpPlus /? (this help)");
      display(string.Empty);
    }
  }
}